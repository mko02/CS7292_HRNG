...
main(int argc, char *argv[])
{
	...
	for( i=1; i<=NUMOFTESTS; i++ ) {
		if (testNames[i] == NULL) {
			fprintf(stderr, "Skipping null testNames[%d]\n", i);
			continue;
		}
		if ( stats[i] != NULL )
			fclose(stats[i]);
...
{ 
	...
	if (generatorDir[option] == NULL) {
		fprintf(stderr, "generatorDir[%d] is NULL! Aborting.\n", option);
		return;
	}
		...
		if ( testVector[i] ) {
			...
			if ( ((i == TEST_CUSUM) && testVector[TEST_CUSUM] ) ||
				...
				for ( k=0; k<numOfFiles; k++ ) {
					if (testNames[i] == NULL) {
						fprintf(stderr, "Skipping NULL test name at index %d (within special-case loop)\n", i);
						continue;
					}
					if ( k < 10 )
				...
				if (testNames[i] == NULL) {
					fprintf(stderr, "Skipping NULL test name at index %d\n", i);
					continue;
				}
				if (generatorDir[option] == NULL) {
					fprintf(stderr, "generatorDir[%d] is NULL! Skipping test %d\n", option, i);
					continue;
				}
...
computeMetrics(char *s, int test)
{
	...
	// CHANGED: Check if testNames[test] is NULL
	const char* test_name = (test >= 0 && test < NUMOFTESTS && testNames[test]) ? testNames[test] : "UNKNOWN TEST";

	if (sampleSize == 0) {
		fprintf(summary, " ------     %s\n", test_name);
	}
	else if (passCount < proportion_threshold_min || passCount > proportion_threshold_max) {
		fprintf(summary, "%4d/%-4d *  %s\n", passCount, sampleSize, test_name);
	}
	else {
		fprintf(summary, "%4d/%-4d    %s\n", passCount, sampleSize, test_name);
	}
